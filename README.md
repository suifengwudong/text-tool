...
项目早期将全部逻辑放在一个 `src/app.rs` 中（~1764 行）。随着功能增加，单文件会带来一系列问题，因此改为 `src/app/` 目录模块。两种方式的对比如下：

| 维度 | 单文件 `src/app.rs` | 目录模块 `src/app/` |
|------|-------------------|--------------------|
...
| **扩展性** | 新增面板/功能需在同一文件里插入代码 | 新增 `panel_xxx.rs` 文件，不影响其他模块 |
